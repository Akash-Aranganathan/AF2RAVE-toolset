...
def generate_biased_plumed(plumedfile,dimension,weights,colvar,height,pace,biasfactor,width,gridmin,gridmax,gridbin):
    f_unb=open(plumedfile)
    f=open('plumed_biased.dat','w')
    lines=f_unb.readlines()
    p=lines.pop(-2)
    w0=",".join([str(weights[0][i]) for i in range (len(weights[0]))])
    lines.insert(-1,f"\nsigma1: COMBINE ARG={colvar} COEFFICIENTS={w0} PERIODIC=NO")
    printline=f"\n PRINT ARG={colvar},sigma1,metad.rbias STRIDE=500 FILE=COLVAR_biased.dat"
    #dimension=np.array(weights).shape[0]
    if dimension == 2:
        w1=",".join([str(weights[1][i]) for i in range (len(weights[1]))])
        lines.insert(-1,f"\nsigma2: COMBINE ARG={colvar} COEFFICIENTS={w1} PERIODIC=NO")
        printline=f"\n PRINT ARG={colvar},sigma1,sigma2,metad.rbias STRIDE=500 FILE=COLVAR_biased.dat"
    lines.insert(-1,f"\nMETAD ...\n \
      LABEL=metad\n \
      ARG=sigma1,sigma2\n \
      ...
      SIGMA={','.join([s for s in width])}\n \
      GRID_MIN={','.join([s for s in gridmin])} GRID_MAX={','.join([s for s in gridmax])} GRID_BIN={','.join([s for s in gridbin])}\n  \
      ... METAD\n")
